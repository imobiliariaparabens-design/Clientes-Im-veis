...
import { toAirtableFields, validatePropertyUpdate } from "@/lib/validation";
  ...
  try {
    ...
    const response = await fetch(airtableUrl, {
      ...
      body: JSON.stringify({
        records: [
          {
            fields: toAirtableFields(validation.payload),
          },
        ],
      }),
    });
    ...
    if (!response.ok) {
      ...
      console.error("Airtable property submission failed", {
